...
- **Consolidate the two font pipelines into one.** `assets/tailwind/extract-font-metrics.ts` + `generate-type-tokens.ts` → `_type-tokens.generated.css` (sizes, line-heights, baseline/line-height *metrics*, optical weight) and the new `assets/fonts/generate-fonts.mjs` (subsetting + `@font-face`) both read the source fonts. `assets/tailwind/` is mislabeled (font-metrics work, not Tailwind). Collapse into one `assets/fonts/` home; `generate-type-tokens` stays a downstream consumer. Own commit. Note: `extract-font-metrics.ts` (fontkit) is **unreliable** for variable + display faces (misreads GT Flexa width 0.21 vs 0.55, Cardinal cap 0.37 vs 0.75) — H1 override values must stay browser-measured, never fontkit-regenerated. **Fold in while here:** the critical-font `<link rel=preload>` in `root.html.heex` hardcodes `/fonts/gt-flexa.woff2` — a duplicate of the body `@font-face` the pipeline emits into `_fontface.generated.css`. Have the pipeline emit the "preload these" list (or a single body-face token the layout reads) so the preload can't drift from the `@font-face` by construction; this removes the double-fetch hazard structurally, making a sync test unnecessary. Also fix the still-broken `assets:font-metrics` npm script (swap `ts-node` for the `esbuild`-bundle+run pattern `generate:typography` already uses) — but do NOT regenerate `font-metrics.json` from the fixed script (fontkit values are wrong, per the note above).
...
- **Footer "(PDF)" vs "Resume" color is still wrong.** The "(PDF)" suffix uses `.footer-muted` = `--text-color-suppressed` (`assets/css/_footer.css`, applied in `footer.ex`). The de-emphasis still doesn't read right — **downstream of the unresolved link/accent + neutral-ramp token decision** (the `/color-tokens` work; note `--accent` is the *darkest* token, so "down a color" went brighter). Settle the token model first, then re-derive the muted/annotation color so "(PDF)" recedes from "Resume" without getting brighter.
