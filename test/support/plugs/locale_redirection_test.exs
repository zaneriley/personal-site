defmodule PortfolioWeb.LocaleRedirectionTest do
  ...
  describe "LocaleRedirection plug" do
    ...
    test "proceeds with request when supported locale is in URL (case-insensitive)", %{conn: conn} do
      for locale <- @supported_locales do
    ...
    test "redirects to user's preferred locale when no locale is in URL", %{conn: conn} do
      preferred_locale = "ja"
      conn =
        conn
        |> init_test_session(%{})
    ...
    test "redirects unsupported locale to default locale", %{conn: conn} do
      ...
      assert_redirected_to(conn, "/#{@default_locale}/case-study/helping-people-find-healthcare")
    end
