defmodule Portfolio.Content.TranslationManagerTest do
  ...
  describe "update_or_create_translation/4" do
    setup do
      case_study = case_study_fixture(%{include_translations: true}) |> Repo.preload(:translations)
      %{case_study: case_study}
    end

    test "creates a new translation when it doesn't exist", %{case_study: case_study} do
      locale = "fr"  # Using French as it doesn't exist in the fixture
      new_metadata = %{
        title: "Titre en français",
        role: "Ingénieur logiciel senior",
        company: "Solutions TechCorp",
        introduction: "Révolutionner la gestion des stocks pour une plateforme e-commerce mondiale.",
        timeline: "Janvier 2022 - Juin 2022"
      }
      new_content = "Contenu en français..."

      updated_translations = TranslationManager.update_or_create_translation(case_study, locale, new_metadata, new_content)

      assert length(updated_translations) == 6  # 5 metadata fields + 1 content field
      ...
      Enum.each(updated_translations, fn {:ok, translation} ->
        ...
        expected_value = if field_name == "content", do: new_content, else: new_metadata[String.to_atom(field_name)]
    ...
    test "updates existing translation when it exists", %{case_study: case_study} do
      ...
      existing_translations = Enum.filter(case_study.translations, & &1.locale == "ja")
      ...
      Enum.each(updated_translations, fn result ->
        case result do
          {:ok, translation} ->
            ...
            assert translation.field_value == expected_value
          {:error, changeset} ->
            IO.puts("Error updating translation for field '#{changeset.changes.field_name}': #{inspect(changeset.errors)}")
            flunk("Failed to update translation for field '#{changeset.changes.field_name}': #{inspect(changeset.errors)}")
        end
      end)
    end
  end
