defmodule Portfolio.Blog.Note do
  ...
  def changeset(note, attrs) do
    ...
    |> validate_length(:title, name: :title_length, max: 255, message: "must be at most 255 characters")
    |> validate_format(:url, ~r/^[a-z0-9\-]+$/,
    ...
    |> generate_url()
    |> unique_constraint(:url, name: :unique_note_urls, message: "URL has already been taken")  end
    ...
    case get_change(changeset, :url) do
      nil ->
        if changeset.valid? do
          ...
          url = title
            |> String.replace(~r/[^\p{L}\p{N}\s-]/u, "") # Remove special characters except spaces and hyphens
