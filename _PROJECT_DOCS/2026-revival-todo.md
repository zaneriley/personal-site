...
**Updated:** 2026-05-18 (DigitalOcean is now the interim origin/deploy substrate until the app and deploy loop are done enough to revisit self-hosting. Real bug-bash evidence proved local Mac cross-arch Docker builds are not trustworthy without Buildx and the 1 GiB DO host cannot build the app without OOM. Deploy work must use prebuilt image digests. The private preview lane has passed repeatedly on the current candidate SHA: validated PR SHA → candidate app image digest → disposable host → runtime viability → preview page acceptance → content publication rehearsal → default destroy.)
...
| 3.5 — Origin substrate + deploy pipeline | 🔄 active | DigitalOcean is the interim substrate until the app and deploy loop are done enough to revisit self-hosting. GitHub Actions remains the deploy operator. Kamal is still not ratified; use the simplest DigitalOcean/Docker path that proves private preview, same-artifact promotion, live smoke, deploy receipts, and rollback. The private preview workflow has passed repeatedly on real PR/candidate SHA runs, including candidate image build, runtime viability, preview page acceptance, content publication rehearsal, artifact safety, artifact upload, and default destroy. Current-SHA repeat proofs: `26023491513` and `26023898742` for PR `90` at `23cdc71df2ce54041a1174a68d57a42a881255bc`; earlier preserved-preview proof `25988301014` was cleaned by sweeper run `25988708619`. The next proof is no longer "repeat preview"; it is choosing the next production-origin/content-promotion slice from this evidence. The simple preview bug bash proved the 1 GiB host OOMs as a builder, so deploy work must consume prebuilt image digests. Static export and self-hosting are parked as later simplification/migration options, not the active path. |
...
3. **Make private preview boring before production origin.** Done for the current proof level: `./run preview:deploy --app-image-ref <digest-ref> --app-sha <sha> --preview-page-acceptance-image <trusted-image> --preview-lease-minutes 15` deploys a digest-pinned candidate image to a disposable host, runs runtime viability, runs preview page acceptance, records lease metadata, and writes `.tmp/ci-artifacts/preview/deploy-receipt.json`. The manual `Private preview deploy` workflow validates the requested PR SHA, builds the candidate image in GitHub Actions, runs that command, and rehearses content publication against the running preview. Current-SHA repeat proofs `26023491513` and `26023898742` passed on PR `90`; both destroyed their disposable hosts with `preserve_preview=false`.
