...
| 3.5 — Origin substrate + deploy pipeline | 🔄 active | DigitalOcean is the interim substrate until the app and deploy loop are done enough to revisit self-hosting. GitHub Actions remains the deploy operator. Kamal is still not ratified; use the simplest DigitalOcean/Docker path that proves preview, same-artifact promotion, live smoke, deploy receipts, and rollback. Manual DigitalOcean create/status/destroy workflow and scripts exist; local create/status/SSH/cloud-init/Docker/destroy passed against a real 1 GiB Droplet, then the Droplet was destroyed. Follow-up hardening added early receipts, failed-create cleanup, ownership-checked destroy, bounded readiness, tag-based listing, default-branch workflow checkout before secrets, and release-image publishing behind `Prod build`. The simple preview bug bash proved the 1 GiB host OOMs as a builder, so deploy work must consume prebuilt image digests. Static export and self-hosting are parked as later simplification/migration options, not the active path. |
