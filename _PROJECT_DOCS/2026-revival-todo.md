...
**Updated:** 2026-05-10 (content authoring front door is active on the feature branches: content-repo CI validates draft safety and app publishability, webhook verdicts record file/reason failures, last-good content stays live on bad pushes, and renames use explicit aliases).
...
2. **Make content deployment deterministic.** Active feature branches now cover delivery dedupe, sync locking, accepted/rejected/ignored SHA recording, last-good boot fallback, content-repo CI, draft safety, and rename aliases. The acceptance test is content deployability, not git freshness: the app must name the live content SHA, the last rejected SHA and reason, the last known-good SHA, and the rollback path. Next: prove the final gates, then add the content-only rollback command and the share-image generation/preview/rendering path.
