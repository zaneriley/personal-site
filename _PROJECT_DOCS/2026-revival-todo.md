...
**Updated:** 2026-05-16 (DigitalOcean is now the interim origin/deploy substrate until the app and deploy loop are done enough to revisit self-hosting. Real bug-bash evidence proved local Mac cross-arch Docker builds are not trustworthy without Buildx and the 1 GiB DO host cannot build the app without OOM. Deploy work must use prebuilt image digests. The current edge is the private preview lane: validated PR SHA → candidate app image digest → disposable host → runtime viability → preview page acceptance → one preview deploy receipt.)
...
| 3.5 — Origin substrate + deploy pipeline | 🔄 active | DigitalOcean is the interim substrate until the app and deploy loop are done enough to revisit self-hosting. GitHub Actions remains the deploy operator. Kamal is still not ratified; use the simplest DigitalOcean/Docker path that proves private preview, same-artifact promotion, live smoke, deploy receipts, and rollback. Lower-level DigitalOcean create/status/destroy debug workflow and scripts exist; local create/status/SSH/cloud-init/Docker/destroy passed against a real 1 GiB Droplet, then the Droplet was destroyed. Follow-up hardening added early receipts, failed-create cleanup, ownership-checked destroy, bounded readiness, tag-based listing, default-branch workflow checkout before secrets, release-image publishing behind `Prod build`, `preview:deploy` as the first top-level private-preview result, and a manual `Private preview deploy` workflow that validates a PR SHA before building and deploying it. The simple preview bug bash proved the 1 GiB host OOMs as a builder, so deploy work must consume prebuilt image digests. Static export and self-hosting are parked as later simplification/migration options, not the active path. |
...
3. **Make private preview boring before production origin.** `./run preview:deploy --app-image-ref <digest-ref> --app-sha <sha> --preview-page-acceptance-image <trusted-image>` is now the intended operator surface: it deploys a digest-pinned candidate image to a disposable host, runs runtime viability, runs preview page acceptance, and writes `.tmp/ci-artifacts/preview/deploy-receipt.json`. The manual `Private preview deploy` workflow validates the requested PR SHA, builds the candidate image in GitHub Actions, then runs that command. Next: prove it against a real PR/host, inspect the receipt as a human, and destroy through `./run preview:destroy`.
