...
**Updated:** 2026-05-17 (DigitalOcean is now the interim origin/deploy substrate until the app and deploy loop are done enough to revisit self-hosting. Real bug-bash evidence proved local Mac cross-arch Docker builds are not trustworthy without Buildx and the 1 GiB DO host cannot build the app without OOM. Deploy work must use prebuilt image digests. The current edge is the private preview lane: validated PR SHA → candidate app image digest → disposable host → runtime viability → preview page acceptance → content publication rehearsal → leased preview cleanup.)
...
| 3.5 — Origin substrate + deploy pipeline | 🔄 active | DigitalOcean is the interim substrate until the app and deploy loop are done enough to revisit self-hosting. GitHub Actions remains the deploy operator. Kamal is still not ratified; use the simplest DigitalOcean/Docker path that proves private preview, same-artifact promotion, live smoke, deploy receipts, and rollback. The private preview workflow has passed once end-to-end on a real PR and disposable host, including candidate image build, runtime viability, preview page acceptance, content publication rehearsal, artifact safety, a reviewable preserved preview, and sweeper cleanup after the preview lease expired. Preserved previews now use a low TTL lease, manual destroy workflow, and scheduled sweeper. The next proof is repeated green previews until the loop is boring. The simple preview bug bash proved the 1 GiB host OOMs as a builder, so deploy work must consume prebuilt image digests. Static export and self-hosting are parked as later simplification/migration options, not the active path. |
...
3. **Make private preview boring before production origin.** `./run preview:deploy --app-image-ref <digest-ref> --app-sha <sha> --preview-page-acceptance-image <trusted-image> --preview-lease-minutes 15` is now the intended operator surface: it deploys a digest-pinned candidate image to a disposable host, runs runtime viability, runs preview page acceptance, records lease metadata, and writes `.tmp/ci-artifacts/preview/deploy-receipt.json`. The manual `Private preview deploy` workflow validates the requested PR SHA, builds the candidate image in GitHub Actions, runs that command, and rehearses content publication against the running preview. One preserved preview was inspected as a human and then destroyed by the sweeper after its lease expired. Next: repeat this on real changes until the loop is boring.
4. **Measure before choosing origin hardware.** Use the preview/runtime vocabulary for live/local runs: time-to-ready, cold-first response, warm p50/p95, memory, CPU, and power where available. The next origin measurement must run a prebuilt image plus Postgres on the 1 GiB DO host and capture runtime memory, not build memory.
5. **Plan origin and edge only after measurements.** The philosophical constraint is smallest honest origin, free/FLOSS where viable, but visitor speed and rollback reliability are the floor.
6. **Plan observability around deploy evidence.** Minimum useful signals: request health, release identity, content SHA, BEAM/runtime health, DB boundary, and cache/origin split after an edge substrate exists.
