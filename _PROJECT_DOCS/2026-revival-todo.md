...
| 3.5 — Origin substrate + deploy pipeline | 🔄 active | DigitalOcean is the interim substrate until the app and deploy loop are done enough to revisit self-hosting. GitHub Actions remains the deploy operator. Kamal is still not ratified; use the simplest DigitalOcean/Docker path that proves private preview, same-artifact promotion, live smoke, deploy receipts, and rollback. The private preview workflow has passed once end-to-end on a real PR and disposable host, including candidate image build, runtime viability, preview page acceptance, content publication rehearsal, artifact safety, and cleanup. Preserved previews now use a low TTL lease, manual destroy workflow, and scheduled sweeper. The next proof is one preserved preview plus cleanup observation, then repeated green previews until boring. The simple preview bug bash proved the 1 GiB host OOMs as a builder, so deploy work must consume prebuilt image digests. Static export and self-hosting are parked as later simplification/migration options, not the active path. |
