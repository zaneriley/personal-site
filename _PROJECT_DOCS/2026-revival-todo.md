...
**Updated:** 2026-05-12 (origin deploy is still in the disposable DigitalOcean host-spike stage, not app deployment. Peer review found lifecycle/provenance gaps; the current branch now hardens receipts, failed-create cleanup, ownership-checked destroy, bounded readiness, default-branch workflow checkout, main-only preview secrets, and release-image gating on `Prod build`. GitHub manual workflow proof still waits until the workflow exists on the default branch).
