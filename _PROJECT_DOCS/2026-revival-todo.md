...
**Updated:** 2026-05-08 (app-side content webhook promotion and optional OG frontmatter persistence implemented; content-repo CI, accepted/rejected SHA ledger, share-image generation, and deleted-URL SEO policy are next).
...
| 3.1 — Content-pipeline sync | 🔄 active | App-side slice now validates repo/ref/after SHA, syncs exact commit, promotes changed Markdown transactionally, rejects bad content without mutating live DB state, unpublishes deleted Markdown, and persists optional OG frontmatter for share-preview steering. Remaining: delivery dedupe, accepted/rejected content-SHA ledger, content-repo CI, boot/startup last-good behavior, OG image generation/validation, and deleted-URL SEO policy. |
...
2. **Make content deployment deterministic.** Partially done app-side: signed payload verification remains delegated to `GitHubWebhook`, and the controller now validates expected repo/ref/after SHA before exact git sync and transactional promotion of affected Markdown. Optional OG frontmatter now persists for share-preview steering. Next: add delivery dedupe/sync locking, record accepted or rejected content SHA, make content-repo CI run the same validation before merge to `main`, and add the OG image generation/preview/rendering path.
