...
**Updated:** 2026-05-08 (app-side content webhook promotion and optional share-preview frontmatter persistence implemented; content deployability now means verdicts, last-good behavior, content identity, content-repo CI, share-image generation, and deleted-URL SEO policy).
...
| 3.1 — Content deployability | 🔄 active | App-side slice now validates repo/ref/after SHA, syncs exact commit, promotes changed Markdown transactionally, rejects bad content without mutating live DB state, unpublishes deleted Markdown, and persists optional share-preview frontmatter. Sync is not the goal by itself. Remaining: delivery dedupe, accepted/rejected content-SHA ledger, content-repo CI, boot/startup last-good behavior, content-only rollback identity, share-image generation/validation, and deleted-URL SEO policy. |
...
2. **Make content deployment deterministic.** Partially done app-side: signed payload verification remains delegated to `GitHubWebhook`, and the controller now validates expected repo/ref/after SHA before exact git sync and transactional promotion of affected Markdown. Optional share-preview frontmatter now persists; the authoring contract lives in `_PROJECT_DOCS/content-authoring-contract.md`. The acceptance test is content deployability, not git freshness: the app must know the live content SHA, the last rejected SHA and reason, the last known-good SHA, and the rollback path. Next: add delivery dedupe/sync locking, record accepted or rejected content SHA, make content-repo CI run the same validation before merge to `main`, and add the share-image generation/preview/rendering path.
